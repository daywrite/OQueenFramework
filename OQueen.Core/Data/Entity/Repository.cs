...
namespace OQueen.Core.Data.Entity
{
    public class Repository<TEntity, TKey> : IRepository<TEntity, TKey> where TEntity : EntityBase<TKey>
    {
        ...
        public IEnumerable<TResult> GetByCondition<TResult>(Expression<Func<TEntity, bool>> predicate, Func<TEntity, TResult> keySelector)
        {
            predicate.CheckNotNull("predicate");
            keySelector.CheckNotNull("keySelector");
            return _dbSet.Where(predicate).Select(keySelector);
        }
